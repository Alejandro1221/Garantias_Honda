...
#Modulos
from Extractores import extraer_numero_factura, extraer_fecha_emision, extraer_valor_total_factura
...
def subir_pdf():
    global paginas_pdf, textos_pdf, indice_actual, archivo_pdf_actual
    ruta = filedialog.askopenfilename(
        title="Seleccionar PDF",
        filetypes=[("Archivos PDF", "*.pdf")]
    )
    if ruta:
        archivo_pdf_actual = ruta
...
def extraer_texto_dpi_alto(ruta_pdf, pagina_index, dpi=350):
    doc = fitz.open(ruta_pdf)
...
def mostrar_pagina(index):
    global paginas_pdf, etiqueta_imagen, zoom_nivel, archivo_pdf_actual
    ...
    #Hacemos OCR a 350 dpi para todo menos para la factura
    texto_dpi_alto = extraer_texto_dpi_alto(archivo_pdf_actual, index, dpi=350)
    area_texto.delete("1.0", tk.END)
    #area_texto.insert(tk.END, textos_pdf[index])
    area_texto.insert(tk.END, texto_dpi_alto)
    ...
    if resultados:
        ...
        campos[20].delete(0, tk.END)
        campos[20].insert(0, ", ".join(referencias))

        campos[21].delete(0, tk.END)
        campos[21].insert(0, ", ".join(descripciones))


    #Extraer dato Factura
    texto_dpi_factura = extraer_texto_dpi_alto(archivo_pdf_actual, index, dpi=150)
    ...
    campos[28].delete(0, tk.END)  # campo "Valor total Factura"
    campos[28].insert(0, valor_total)
